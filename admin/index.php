...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Fetch Doctor Demand Data
        fetch("get_doctor_demand.php")
            .then(response => response.json())
            .then(data => {
                const doctorNames = data.map(d => d.docname);
                const appointmentCounts = data.map(d => d.total_appointments);

                const ctx = document.getElementById("doctorDemandChart").getContext("2d");
                new Chart(ctx, {
                    type: "bar",
                    data: {
                        labels: doctorNames,
            ...
            .catch(error => console.error("Error loading doctor demand data:", error));

        // Fetch Specialty Demand Data
        fetch("get_specialty_demand.php")
            .then(response => response.json())
            .then(data => {
                const specialties = data.map(d => d.specialty);
                const appointmentCounts = data.map(d => d.total_appointments);

                const ctx = document.getElementById("specialtyDemandChart").getContext("2d");
                new Chart(ctx, {
                    type: "pie",
                    data: {
                        labels: specialties,
                        datasets: [{
                            label: "Appointments per Specialty",
                            data: appointmentCounts,
            ...
            .catch(error => console.error("Error loading specialty demand data:", error));
    });
</script>

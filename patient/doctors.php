...
// Update the query to include a JOIN between `doctor` and `specialties`
$list11 = $database->query("
    SELECT doctor.docname, doctor.docemail, specialties.sname 
    FROM doctor 
...
if ($list11) {
    for ($y = 0; $y < $list11->num_rows; $y++) {
        ...
        $docName = htmlspecialchars($row00["docname"], ENT_QUOTES, 'UTF-8');
        $docEmail = htmlspecialchars($row00["docemail"], ENT_QUOTES, 'UTF-8');
        $specialty = htmlspecialchars($row00["sname"], ENT_QUOTES, 'UTF-8');

        // Add each doctor name, email, and specialty as options in the datalist
...
if ($_POST) {
    ...
    // Modified query to include a JOIN with the specialties table
    $sqlmain = "
        SELECT doctor.*, specialties.sname 
        FROM doctor 
        LEFT JOIN specialties ON doctor.specialties = specialties.id 
        WHERE doctor.docemail = '$keyword' 
        OR doctor.docname = '$keyword' 
